...
public class AssertionMethods extends SelectElementByType {
     ...
     * @return String
     */
    public Set<String> getElementsTexts(String accessType, String accessName) {
        List<WebElement> elements = getAllWebElements(accessType, accessName);
        Set<String> texts = new HashSet<>(elements.size());
    ...
    public void checkElementText(String accessType, String actualValue, String accessName, boolean testCase) throws TestCaseFailed {
        Set<String> elementTexts = getElementsTexts(accessType, accessName);

        if (testCase) {
            if (!elementTexts.contains(actualValue)) {
                throw new TestCaseFailed("Text Not Matched, actual value is '" + actualValue + "', expected " + elementTexts);
            }
        } else {
            if (elementTexts.contains(actualValue)) {
     ...
     * @return String
     */
    public Set<String> getElementsAttribute(String accessType, String accessName, String attributeName) {
        List<WebElement> elements = getAllWebElements(accessType, accessName);
    ...
    private List<WebElement> getAllWebElements(String accessType, String accessName) {
        return wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(getElementByType(accessType, accessName)));
    }
     ...
     * @param testCase       : Boolean : test case [true or false]
     */
    public void checkElementsAttribute(String accessType, String attributeName, String attributeValue, String accessName, boolean testCase) throws TestCaseFailed {
        Set<String> attrValues = getElementsAttribute(accessType, accessName, attributeName);
        if (testCase) {
            if (!attrValues.contains(attributeValue)) {
                throw new TestCaseFailed("Attribute Value Not Matched");
            }
        } else {
            if (attrValues.contains(attributeValue)) {
    ...
    public void isOptionFromRadioButtonGroupSelected(String accessType, String by, String option, String accessName, boolean shouldBeSelected) throws TestCaseFailed {
        List<WebElement> radioButtonGroup = getAllWebElements(accessType, accessName);
